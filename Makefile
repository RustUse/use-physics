...
FOCUSED_CRATES := use-motion use-rotation use-force use-torque use-energy use-work use-power use-fluid use-electricity use-magnetism use-electromagnetism use-plasma use-pressure use-density use-gravity use-orbit use-momentum use-relativity use-quantum use-particle use-nuclear use-thermodynamics
...
help:
	@printf "%s\n" \
		...
		"deny                           Run cargo-deny" \
		"sbom                           Generate a CycloneDX SBOM for use-physics" \
		"publish-dry-run-focused        List package contents and dry-run publish focused crates" \
		"publish-dry-run-facade         Dry-run publish use-physics after crates.io propagation" \
...
sbom:
	cargo cyclonedx --manifest-path crates/use-physics/Cargo.toml --all-features --format json --spec-version 1.5 --override-filename sbom.cyclonedx
...
publish-dry-run-facade:
	cargo publish --dry-run --allow-dirty -p use-physics
