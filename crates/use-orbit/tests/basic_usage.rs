...
#[test]
fn orbital_helpers_cover_common_transfer_workflow() {
    ...
    let low_orbit_period = earth.and_then(|body| {
        low_orbit_radius.and_then(|radius| body.circular_orbital_period_at_radius(radius))
    });
    ...
    assert!(low_orbit_period.is_some_and(|period| period > 0.0));
    assert!(transfer.is_some_and(|delta_v| delta_v > 0.0));
    assert!(
        EllipticalOrbit::new(100.0, 10.0, 20.0)
            .and_then(|orbit| orbit.periapsis_speed())
